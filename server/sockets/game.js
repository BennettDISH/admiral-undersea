...
function setupGameSockets(io) {
  ...
  function scheduleSonarTimeout(gameCode, respondingTeam) {
    setTimeout(() => {
      try {
        const cur = gameStates.get(gameCode);
        const sub = cur?.submarines?.[respondingTeam];
        if (!sub?.pendingSonar) return; // already answered
  ...
  // the stragglers after a timeout so the captain isn't stuck forever.
  function scheduleTurnWatchdog(gameCode, team) {
    setTimeout(() => {
      try {
        const cur = gameStates.get(gameCode);
        const sub = cur?.submarines?.[team];
        if (!sub || !sub.awaitingConfirmation) return;
        const all = [];
  ...
  io.on('connection', (socket) => {
    ...
    on('captain-move', async ({ gameCode, direction }) => {
      ...
      if (state.mode !== 'live') scheduleTurnWatchdog(gameCode, team);
    });
    ...
    on('use-sonar', async ({ gameCode }) => {
      ...
      if (apply(socket, gameCode, state, engine.useSonar(state, team))) {
        scheduleSonarTimeout(gameCode, enemyTeam);
      }
    });
